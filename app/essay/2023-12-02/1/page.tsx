...
`}</SyntaxHighlighter>
        ...
        <h3>侧边栏</h3>
        <p>
          后台总体框架的搭建和交通局的类似，但是我这次采用了重邮帮后台的路由以及侧边栏渲染，这里就不多谈了，有兴趣可以看
          <a href="https://www.yygod0120.com/2023/10/24/%E2%88%A0%E7%97%9B%E5%B1%80/">
            交通局
          </a>
        </p>
        <h3>数据可视化界面</h3>
        <p>
          数据可视化界面中的图表，我采用的是
          <a href="https://recharts.org/en-US/">Rechart</a>
          ,易上手，类型丰富，最重要的是有中文翻译
        </p>
        <p>
        ...
        <h4>service</h4>
        <p>
          先说 service，我们技术选型的时候选择了 SWR
          作为网络请求库,原因有以下几点：
        </p>
        <ol>
          <li>
            SWR
            具有自动重新请求以及用户重新聚集时请求，这有助于数据实时更新以及展示
          </li>
          <li>
            SWR 能避免很多 useEffect
            中编写网络请求遇到的问题，比如说请求顺序和返回顺序的不一致以及依赖相关的问题。
          </li>
          <li>完美符合了数据组件分离政策</li>
...
`}</SyntaxHighlighter>
        <p>
          像这样一个 swr 获取 PV 数据，在 isLoading
          阶段是不会有返回值的，这样就会导致
          <code>const [cqappPv, cqhelperPv] = useGetPv()</code>报错 undefined
          所以我采用<code>return [null,null]</code>
          来处理,像这样的空值处理在数据组件分离中很常见，感觉我这样的写法也不算是最优的。
        </p>
        <p>
          另外一点关于 service 数据处理的是关于类型的处理 早在
          <a href="https://www.yygod0120.com/2023/09/18/TheFirstRedrockProject/">
            暑假的场地申请
          </a>
          中遇到两个模块类似的数据的处理，当时我的做法是直接拆开为两个部分，但是这样会有一些问题：
        </p>
        <ul>
...
`}</SyntaxHighlighter>
        ...
        <ul>
          <li>
            第一层：对 Rechart
            中的折线图，饼图以及柱状图进行封装，让图本身符合要求
          </li>
          <li>
            第二层：对封装后的 Rechart 再进行 Header 以及 Bottom
            的封装，让图&quot;更完整&quot;
          </li>
        </ul>
        <p>为了文章的大小就不在这展示了。</p>
        ...
        <p>
          这算是第 4
          次做项目，可能也是因为项目本身也不大，做起来还是很快的，只是在后端接口上耽误了时间
        </p>
        <p>
          debug 能力也有所提高，对于一些小 bug
          比如跨域以及图片路径丢失等等，解决起来比暑假快上不少
        </p>
        <p>
          重点收获是图表的封装以及对网络请求的一些理解，还有 React Design
          的一些想法
        </p>
        <blockquote>
          <p>记得关注 RRFE-CLI</p>
