    ...
    <div>
      <div className="mt-8 flex flex-col items-start rounded-sm bg-white text-lg shadow-lg">
        ...
        <div className="flex w-[95vw] flex-col break-words px-[2.5vw] pb-12 text-start md:w-[90vw] md:px-[2.5vw] lg:w-[740px] lg:px-20">
          ...
          <p>
            React
            开发人员可以通过使用设计模式来节省时间和精力，这些模式提供了一种使用经过测试和信任的解决方案解决问题的快速方法。它们使内聚模块具有较低的耦合性，这反过来又帮助
            React
            开发人员创建可维护、可扩展和高效的应用程序。在本文中，我们将探讨
            React 设计模式，并研究它们如何改进 React 应用程序的开发。
          </p>
          <h1>容器和表示模式</h1>
          <p>
            容器和表示模式是一种模式，旨在将表示逻辑与反应代码中的业务逻辑分离，从而使其模块化、可测试，并遵循关注分离原则。
            大多数情况下，在 react
            应用程序中，我们需要从后端/存储中获取数据或计算逻辑，并在 react
            组件上表示该计算的结果。在这些情况下，容器和表示模式大放异彩，因为它可用于将组件分为两个，即：
          </p>
          <ul>
...
`}</SyntaxHighlighter>
          ...
          <p>
            钩子是 React 16.8 中首次亮相的全新功能。从那时起，他们在开发 react
            应用程序方面发挥了至关重要的作用。钩子是基本函数，用于授予功能组件对状态和生命周期方法的访问权限（以前仅供类组件使用）。另一方面，钩子可以专门设计来满足组件要求并具有其他用例。
...
`}</SyntaxHighlighter>
          <p>
            创建自定义钩子后，我们会将其导入到我们的 StarWarsCharactersContainer
            组件中并使用它:
          </p>
          <SyntaxHighlighter
            language="tsx"
...
`}</SyntaxHighlighter>
          ...
          <p>
            大多数情况下，处理组件中的许多状态会导致许多未分组状态的问题，这可能很麻烦且难以处理。在这种情况下，Reducers
            模式可能是一个有用的选择。我们可以使用 reducer
            将状态分类为某些操作，这些操作在执行时可以更改分组状态。
...
`}</SyntaxHighlighter>
          ...
          <ul>
            <li>
              “login”操作类型会触发状态更改，该更改会影响三个状态值，即
              loggedIn、user、token。
            </li>
          ...
          <h1>使用提供程序进行数据管理</h1>
          <p>
            提供程序模式对于数据管理非常有用，因为它利用上下文 API
            通过应用程序的组件树传递数据。这种模式是 Props drilling
            的有效解决方案，Props drilling 一直是 React 开发中常见的问题。
            为了实现提供程序模式，我们将首先创建一个提供程序组件。提供程序是
            Context 对象提供给我们的高阶组件。我们可以利用 React 提供的
            createContext 方法构造一个 Context 对象。
          </p>
          <SyntaxHighlighter
            language="tsx"
            style={oneLight}
...
`}</SyntaxHighlighter>
          <p>
            创建提供程序后，我们将使用创建的提供程序组件将依赖于上下文 API
            中数据的组件封闭起来。 为了从上下文 API 获取数据，我们调用
            useContext 钩子，它接受上下文作为参数（在本例中为 ThemeContext）。
          </p>
          <SyntaxHighlighter
            language="tsx"
            style={oneLight}
...
`}</SyntaxHighlighter>
          ...
          <p>
            高阶组件将组件作为参数，并返回注入了附加数据或功能的增压组件。React
            中 HOC 的可能性是由于 React 对组合的偏好而不是继承。 高阶组件
            （HOC）
            模式提供了一种增加或修改组件功能的机制，从而促进了组件重用和代码共享。
            HOC 模式的示例如下所示：
          </p>
          <SyntaxHighlighter
            language="tsx"
...
`}</SyntaxHighlighter>
          ...
          <p>
            ...
            这种模式背后的原理是将父组件分解为更小的组件，然后使用道具、上下文或其他
            React 数据管理技术来管理这些较小组件之间的交互。
            当需要创建由较小组件组成的可重用的多功能组件时，此模式会派上用场。它使开发人员能够创建复杂的
            UI 组件，这些组件可以轻松自定义和扩展，同时保持清晰简单的代码结构。
            复合组件模式的用例示例如下所示：
          </p>
          <SyntaxHighlighter
            language="tsx"
...
`}</SyntaxHighlighter>
          <h1>Prop 组合</h1>
          <p>
            这需要从几个相关的 prop 中创建一个对象，并将其作为单个 prop
            传递给组件。
...
`}</SyntaxHighlighter>
          ...
          <p>
            ...
            由于 React
            控制组件的状态和行为，因此这种模式使代码比不受控制的输入模式更具可预测性和可读性，后者不使用组件的状态，而是直接通过
            DOM（文档对象模型）来控制它。 受控输入模式的用例示例如下所示：
          </p>
          <SyntaxHighlighter
            language="tsx"
            style={oneLight}
...
`}</SyntaxHighlighter>
          ...
          <p>
            称为 ForwardRef
            的高阶组件将另一个组件作为输入，并输出一个传递原始组件的 ref
            的新组件。通过这样做，子组件的 ref（可用于检索底层 DOM
            节点或组件实例）可供父组件访问。
            创建与第三方库或应用程序中的其他自定义组件交互的自定义组件时，在工作流中包含
            ForwardRef 模式非常有帮助。通过授予对库的 DOM 节点或其他组件的 DOM
            实例的访问权限，它有助于将此类组件的控制权转移给您。 下面显示了
            forwardRef 模式的用例示例：
          </p>
          <SyntaxHighlighter
            language="tsx"
...
`}</SyntaxHighlighter>
          <p>
            在上面的代码中，我们 <code>&lt;CustomInput/&gt;</code>{" "}
            <code>&lt;ParentComponent/&gt;</code> 使用 forwardRefs .
          </p>
          <h1>总结</h1>
          <p>
            我们在本文中讨论了 React
            设计模式，包括高阶组件、容器表示组件模式、复合组件、受控组件等等。通过将这些设计模式和最佳实践整合到您的
            React
            项目中，您可以提高代码质量、促进团队协作，并使您的应用程序更具可扩展性、灵活性和可维护性。
          </p>
        </div>
      </div>
