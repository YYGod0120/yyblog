    ...
    <div>
      <div className="mt-8 flex flex-col items-start rounded-sm bg-white text-lg shadow-lg">
        ...
        <div className="flex w-[95vw] flex-col break-words px-[2.5vw] pb-12 text-start md:w-[90vw] md:px-[2.5vw] lg:w-[740px] lg:px-20">
          ...
          <p>
            原本，我利用 Hexo
            搭建了一个个人博客，刚开始还挺新鲜的，但慢慢就觉得样式单调，而且我想加入评论以及流量监控很麻烦。所以想着要不自己手写一个，更好控制也能自定义样式。于是
            yyblog 就孕育而生
          </p>
          <h1>结构</h1>
          <p>
            整个博客，不仅仅由 yyblog 组成，还有 ybg-cli
            脚手架，用于自动创建删除文章以及编译文章。
          </p>
          <ul>
            <li>
              <p>
                <strong>yyblog</strong>
              </p>
              <ul>
                <li>Nextjs</li>
                <li>typescript</li>
          ...
          <h2>yyblog</h2>
          <p>
            采用<code>Nextjs</code>+<code>Typescript</code>+
            <code>Tailwindw</code>为主要技术。
          </p>
          <p>
            效仿 Hexo
            采用纯前端，文章编写删除编译都在本地运行，对前端工程师更友好。
            Nextjs 同时也支持全栈开发。简单的 sql 语句也能够对文章进行增删改查。
          </p>
          <p>
            Next 提供的文件路由同时也可以帮我们自定义一些特殊页面，常见的 layout
            页面，404 页面以及错误页面等等。
          </p>
          <blockquote>
            <p>避免中文命名文件！！</p>
          ...
          <p>
            除了文件路由以外，Next 也提供了很多组件以及优化，比如{" "}
            <code>font</code>，<code>&lt;Image&gt;</code>标签还有
            <code>&lt;Script&gt;</code>标签。 这里说一下
            <code>&lt;Script&gt;</code>，博客网站上我放了一些动画(来源于
            codepen),而这些动画<code>script</code>脚本，放入{" "}
            <code>tsx(jsx)</code> 中只能通过<code>dangerouslySetInnerHTML</code>
            这个极具风险的 attribute 但正好 Nextjs 提供了一个
            <code>&lt;Script&gt;</code>，能够让我们使用脚本组件。
          </p>
          <h2>ybg-cli</h2>
          <p>
            采用纯前端的重点就在于文章的生成删除以及编译，麻烦就麻烦在编译成 TSX
            代码插入到 yyblog 中形成页面。
          </p>
          <p>光看文字不如上点代码：</p>
          <SyntaxHighlighter
...
`}</SyntaxHighlighter>
          <p>
            ...
            <code>other</code>是解析成 Html 后需要插入到最后 TSX 内容的东西。
          </p>
          <p>
            重点在于<code>HtmlToNext</code>函数:
          </p>
          <SyntaxHighlighter
            language="tsx"
            style={oneLight}
...
`}</SyntaxHighlighter>
          <p>
            通过对解析后 html 的修改，主要是用正则，实现 html 向
            TSX(Next)的转化。
          </p>
          <h1>3月4号更新：</h1>
          <p>
            更新了英文版，主要采用的是Next的中间件-<code>middleware.js</code>
            以及<code>react-i18next</code>和<code>i18next</code>
            ,这两个库i18n转化库。
          </p>
          <p>
            配置的教程在
            <a href="https://locize.com/blog/next-app-dir-i18n/">这里</a>
          </p>
          <SyntaxHighlighter
            language="javascript"
            style={oneLight}
...
`}</SyntaxHighlighter>
          <p>
            <code>matcher</code>匹配器用于使得中间件在特定的路径上执行。
          ...
          <p>
            ...
            在根目录下设置<code>_blog.json</code>,配置文件：
          </p>
          <SyntaxHighlighter
            language="json"
            style={oneLight}
...
`}</SyntaxHighlighter>
          <p>
            然后使用<code>ybg init</code>进行初始化。 最后每次写完文章执行
            <code>pnpm d</code>进行部署。
          </p>
          <h1>附件</h1>
          <blockquote>
            <p>一些使用到的库或者代码：</p>
          ...
          <ul>
            <li>
              <a href="https://github.com/jonschlinkert/gray-matter">
                gray-matter
              </a>
              ：解析静态网页元数据
            </li>
            <li>
              <a href="https://github.com/markedjs/marked">marked</a>：转化
              <code>md</code>文件为 HTML
            </li>
            <li>
            ...
            <li>
              <a href="https://github.com/egoist/tsup">tsup</a>：快速构建
              TypeScript 项目的工具
            </li>
            <li>
              <a href="https://github.com/alexeyraspopov/picocolors">
                picocolors
              </a>
              ：命令行颜色
            </li>
            <li>
              <a href="https://github.com/cacjs/cac">cac</a>：构建命令行工具的
              JavaScript/TypeScript 框架
            </li>
            <li>
              <a href="https://codepen.io/">codepen</a>： 动画及特别的 404
              页面都来源于此
            </li>
            <li>
              <a href="https://github.com/i18next/react-i18next">
                react-i18next
              </a>{" "}
              i18n for react
            </li>
